...
def ejecutar_prediccion_rentabilidad(entrenar_modelo):
    ...
    #----------- QUIERES USAR MODELO PREENTRENADO O ENTRENAR????
    # if entrenar_modelo:
    #     best_knn_model = rentPriceRegressorFunciones.entrenarModelo(X_train, y_train_log)
    #     pred_log = best_knn_model.predict(X_test) #SIN USAR FUNCIÓN PREDECIR CONJUNTO TEST
    # else:
    #     # Cargar el modelo preentrenado
    #     modelo_entrenado = joblib.load('modelo_entrenado.pkl')
    #     pred_log = modelo_entrenado.predict(X_test)
    #     pass
