...
def main():
    ...
    # ubicacion_archivo = "./datasetsRent/fusionado_con_interest_points.csv"
    # fusionar_csv(ubicacion_archivo, "./datasetsRent/buscarNuevos/datos_idealistaNuevos.csv")
    ...
    # separar_train_test(ubicacion_archivo, train_path, test_path)
    
    # fusionar_csv("./rentdatos/rent24may.csv", "./rentdatos/rent12abr.csv")
    
    # fusionar_csv("./rentdatos/rent12abr.csv", "./rentdatos/rent3feb.csv")
    
    
    
    
    
    #prueba de quitar pueblos y outliers con clustering antes de separar en train test
    
    # file_path = './rentdatos/todosRent.csv'
    # todosRent = pd.read_csv(file_path, na_values="NaN")
    # print (todosRent.shape)
    # print (todosRent.shape)
    # print(f"Posibles valores en la columna municipality ANTES:")
    # unique_values = todosRent['municipality'].value_counts()
    # print(unique_values)
    # print("\n" + "="*50 + "\n") 
    # todosRent = soloGranadaCiudad(todosRent)
    
    # print (todosRent.shape)
    # print(f"Posibles valores en la columna municipality DESPUÉS:")
    # unique_values = todosRent['municipality'].value_counts()
    # print(unique_values)
    # print("\n" + "="*50 + "\n")  # Separador para mayor claridad
    
    # path_nuevo= "./rentdatos/soloGranada.csv"
    # todosRent.to_csv(path_nuevo, index=False)
    
    # calcularDistanciaPuntosInteres(path_nuevo)
    # file_path_solo_granada= "./rentdatos/soloGranada.csv"
    # todosRent = pd.read_csv(file_path_solo_granada, na_values="NaN")
    
    # todosRent = preprocesartodo(todosRent)
    
    # visualizarDatos(todosRent)
    
    # print (todosRent.shape)
    
    # todosRent.to_csv("./rentdatos/todospreprocesado.csv", index=False)
    
    
    # file_path_preprocesado = './rentdatos/todospreprocesado.csv'
    # todosRent = pd.read_csv(file_path_preprocesado, na_values="NaN")
    
    # elbow_method_kmeans(todosRent)
    
    # file_path_sin_outliers = quitarOutliersClustering(file_path_preprocesado, file_path_solo_granada)
    
    # print(f"El archivo preprocesado se ha guardado en: {file_path_sin_outliers}")
    
    # train_path = './datasetsRent/train_dataset_interest_points.csv'  
    # test_path = './datasetsRent/test_dataset_interest_points.csv' 

    # #aquí hacer que el csv path_nuevo quite las instancias que ha quitado el clustering en file_path_sin_outliers. Ten en cuenta que las columnas de los dos csv van a cambiar en la mayoría de los casos
    # # separar_train_test(file_path_sin_outliers, train_path, test_path)
    # separar_train_test("./rentdatos/soloGranada.csv", train_path, test_path)
   ...
   # ---------PRUEBA MODELO SOLO---------
    ...
    # separar_train_test("./rentdatos/rent24may.csv", "./rentdatos/train.csv", "./rentdatos/test.csv")
    
    # calcularDistanciaPuntosInteres("./rentdatos/antiguoNuevasDistancias/train.csv")
    # calcularDistanciaPuntosInteres("./rentdatos/antiguoNuevasDistancias/test.csv")
